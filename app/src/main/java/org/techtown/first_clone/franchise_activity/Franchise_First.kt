...
class Franchise_First : AppCompatActivity() {
    private lateinit var binding:ActivityFranchiseFirstBinding
    ...
    override fun onResume() {
        ...
        CategoryAdapter.setItemClickListener(object: First_Category_Adapter.OnItemClickListener{
            override fun onClick(v: View, position:Int){
                ...
                (binding.FirstContentRecyclerview.layoutManager as LinearLayoutManager).scrollToPositionWithOffset(position,0)
                (binding.FirstRecyclerView.layoutManager as LinearLayoutManager).scrollToPositionWithOffset(position,binding.FirstRecyclerView.width/2)
                //scrollToPositionWithOffset같은 경우는 LinearLayoutManager에서 제공하는 메소드이기 떄문에 LinearLayoutManager롤 캐스팅을 해서 참조했다.
                // 두번쨰로 들어가는 파라미터는 왼쪽가장자리에서 얼마나 거리를 벌릴 지에 대한 값이다.(offset)
            }
        })
